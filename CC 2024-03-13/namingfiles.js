...
// there has to be a way to condense this. i wanted to use Array
function nameFile(fmt, nbr, start) {
    if (!Number.isInteger(start) || nbr < 1) {
        return []
    }
    let arr = []
    for (let i = start; i < nbr+start; i++) {
        let file = fmt.replaceAll('<index_no>', i)
        arr.push(`${file}`)
    }
    return arr
...
// condesned into a one liner
function nameFile(fmt, nbr, start) {
    return !Number.isInteger(start) || nbr < 1 ? [] : Array(nbr).fill(fmt).map((x, i) => x.replace('<index_no>', start + i));
}