...
function decode  (code, n) {
    const key = String(n)
    code = code.map((x,i) => x - Number(key[i % key.length]))
    return String.fromCharCode(...(code.map(x => x+96)))
}

// same function and same method, but the 2nd condenses 2 lines into one. you can map and add 96 to each code at the same time as subtracting the key numbers.
